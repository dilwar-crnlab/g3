...
monitor_info_keywords = (
    ...
    'service_blocking_rate_since_reset',
    'bit_rate_blocking_rate',
    'bit_rate_blocking_rate_since_reset'
)
...
env_args = dict(scenario=scenario, load=100, topology=topology, seed=10, #load is for traffic load
                allow_rejection=False, # the agent cannot proactively reject a request
                j=1, # consider only the first suitable spectrum block for the spectrum assignment
                mean_service_holding_time=100, # value is not set as in the paper to achieve comparable reward values
                #num_spectrum_resources=num_spectrum_resources, not required set in RBMSAEnv class
...
env = gym.make('DeepRBMSA-v0', **env_args)
...
agent = DQN(
    ...
    learning_rate=2.18177e-05,
    buffer_size=1000000,  # Size of replay buffer
    learning_starts=500,  # How many steps before starting learning
    batch_size=128,
    gamma=0.956124,  # Discount factor
    exploration_fraction=0.2842255,  # Fraction of total timesteps for exploration
    exploration_initial_eps=0.975309,  # Initial exploration probability
    exploration_final_eps=0.0778091,  # Final exploration probability
    target_update_interval=468,  # How often to update target network
    train_freq=8,  # Update the model every 4 steps
    gradient_steps= 6
)
