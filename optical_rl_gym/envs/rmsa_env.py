...
class RMSAEnv(OpticalNetworkEnv):
    ...
    def __init__(
        ...
        # Initialize topology transformation
        self.topology_transform = TopologyTransform(self.topology)
        self.spectrum_availability = SpectrumAvailability(self.num_spectrum_resources)

        # Add transformed topology attributes to the original topology
        self._init_transformed_topology_attributes()
    ...
    def _get_path_spectrum_metrics(self, path: Path) -> dict:
        """Calculate spectrum metrics for a path"""
        # Calculate required slots for this path
        required_slots = self.get_number_slots(path)
        
        # Get spectrum metrics
        return self.spectrum_availability.get_metrics(
            path=path,
            topology=self.topology,
            required_slots=required_slots
        )
    
    def _get_all_paths_spectrum_metrics(self) -> dict:
        """Get spectrum metrics for all k-shortest paths of current service"""
        ...
        num_paths = len(paths)
        metrics = {
            'nfs': np.zeros(num_paths, dtype=np.int32),
            'nfsb': np.zeros(num_paths, dtype=np.int32),
            'nfsb_valid': np.zeros(num_paths, dtype=np.int32),
            'istart': np.full(num_paths, -1, dtype=np.int32),
            'sfirst': np.full(num_paths, -1, dtype=np.int32),
            'sfsb': np.zeros(num_paths, dtype=np.float32)
        }
        
        # Calculate metrics for each path
        for idx, path in enumerate(paths):
            path_metrics = self._get_path_spectrum_metrics(path)
            for key in metrics:
                metrics[key][idx] = path_metrics[key]
                
        return metrics

    def _init_rate_histograms(self):
        """Initialize histograms for discrete bit rate tracking"""
        self.bit_rate_requested_histogram = defaultdict(int)
        self.bit_rate_provisioned_histogram = defaultdict(int)
        self.episode_bit_rate_requested_histogram = defaultdict(int)
        self.episode_bit_rate_provisioned_histogram = defaultdict(int)
    ...
    def _get_source_destination_tau(self):
        """Create one-hot encoding for source and destination nodes"""
        tau = np.zeros((2, self.topology.number_of_nodes()))
        tau[0, self.current_service.source_id] = 1
        tau[1, self.current_service.destination_id] = 1
        return tau
    
    def _get_path_features(self, path: Path) -> np.ndarray:
        """Get features for a specific path"""
        # Get available slots across the path
        available_slots = self.get_available_slots(path)
        
        # Get required slots for this path
        required_slots = self.get_number_slots(path)
        
        # Calculate path features
        features = np.zeros(self.num_spectrum_resources)
        for i in range(len(path.node_list) - 1):
            edge_idx = self.topology[path.node_list[i]][path.node_list[i + 1]]['index']
            edge_slots = self.topology.graph['available_slots'][edge_idx]
            features = np.minimum(features, edge_slots)  # Element-wise minimum
    ...
    def step(self, action):
        ...
        if (path < self.k_paths and initial_slot < self.num_spectrum_resources):  # action is for assigning a path
            ...
            #slots = self.get_number_slots(self.k_shortest_paths[self.current_service.source, self.current_service.destination][path])

            # Get path and calculate required slots
            selected_path = self.k_shortest_paths[self.current_service.source, self.current_service.destination][path]
            ...
            path_metrics = self._get_path_spectrum_metrics(selected_path)

            # Check if this slot is in a valid block
            if (path_metrics['istart'] <= initial_slot and 
                initial_slot + slots <= path_metrics['istart'] + path_metrics['sfirst']):
                # Provision the path
                self._provision_path(selected_path, initial_slot, slots)
                self.current_service.accepted = True
                self.actions_taken[path, initial_slot] += 1
                self.episode_actions_taken[path, initial_slot] += 1
                if (self.bit_rate_selection == "discrete"):  # if discrete bit rate is being used
        ...
        if not self.current_service.accepted:
            ...
            self.episode_actions_taken[self.k_paths, self.num_spectrum_resources] += 1
    ...
    def reset(self, only_episode_counters=True):
        ...
        # Reset episode action tracking arrays
        self.episode_actions_output = np.zeros(
            (
                self.k_paths + self.reject_action,
                self.num_spectrum_resources + self.reject_action,
        ...
        # Reset histograms for discrete bit rate selection
        if self.bit_rate_selection == "discrete":
            self.episode_bit_rate_requested_histogram = defaultdict(int)
            self.episode_bit_rate_provisioned_histogram = defaultdict(int)
            self.episode_slots_requested_histogram = defaultdict(int)
        ...
        if only_episode_counters:
            if self._new_service:
                # Initialize episode counters when current service remains
                ...
                if self.bit_rate_selection == "discrete":
                    ...
                    # Build histogram of slots requested for shortest path
        ...
        # Reset histograms for discrete bit rate
        if self.bit_rate_selection == "discrete":
            self.bit_rate_requested_histogram = defaultdict(int)
            self.bit_rate_provisioned_histogram = defaultdict(int)
        
        # Reset topology metrics
        ...
        # Reset GCN/RNN specific attributes
        self.topology_transform = TopologyTransform(self.topology)
        self._init_transformed_topology_attributes()
        self.spectrum_availability = SpectrumAvailability(self.num_spectrum_resources)
    ...
    def _provision_path(self, path: Path, initial_slot, number_slots):
        ...
        for i in range(len(path.node_list) - 1):
            ...
            self.topology.graph['available_slots'][edge_idx, initial_slot:initial_slot + number_slots] = 0
            self.spectrum_slots_allocation[edge_idx,initial_slot : initial_slot + number_slots,] = self.current_service.service_id
    ...
    def _release_path(self, service: Service):
        ...
        for i in range(len(service.path.node_list) - 1):
            ...
            self.topology.graph["available_slots"][edge_idx,service.initial_slot : service.initial_slot + service.number_slots,] = 1
            self.spectrum_slots_allocation[edge_idx,service.initial_slot : service.initial_slot + service.number_slots,] = -1
    ...
    def _next_service(self):
        ...
        # print("MSIAT", self.mean_service_inter_arrival_time)
        # print("MSHT", self.mean_service_holding_time)
        ...
        bit_rate = (self.bit_rate_function() if self.bit_rate_selection == "continuous" else self.bit_rate_function()[0])
    ...
    def get_number_slots(self, path: Path) -> int:
        ...
        """
        return (math.ceil(self.current_service.bit_rate/ (path.best_modulation.spectral_efficiency * self.channel_width))+ 1)
    ...
    def is_path_free(self, path: Path, initial_slot: int, number_slots: int) -> bool:
        ...
        for i in range(len(path.node_list) - 1):
            if np.any(self.topology.graph["available_slots"][
                    self.topology[path.node_list[i]][path.node_list[i + 1]]["index"],initial_slot : initial_slot + number_slots,]== 0):
                return False
        return True
    ...
    def get_available_blocks(self, path):
        ...
        # zero if not
        available_slots = self.get_available_slots(self.k_shortest_paths[self.current_service.source, self.current_service.destination][path])
