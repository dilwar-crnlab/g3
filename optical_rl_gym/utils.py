...
def evaluate_heuristic(
    ...
    for episode in range(n_eval_episodes):
        ...
        print(f"Services Processed: {env.episode_services_processed}")
        print(f"Services Accepted: {env.episode_services_accepted}")
        if env.episode_services_processed > 0:
            blocking_rate = (env.episode_services_processed - env.episode_services_accepted) / env.episode_services_processed
            print(f"Blocking Rate: {blocking_rate:.4f}")
        
        # Print bit rate statistics
        print(f"Bit Rate Requested: {env.episode_bit_rate_requested}")
        print(f"Bit Rate Provisioned: {env.episode_bit_rate_provisioned}")
        if env.episode_bit_rate_requested > 0:
            bit_rate_blocking = (env.episode_bit_rate_requested - env.episode_bit_rate_provisioned) / env.episode_bit_rate_requested
            print(f"Bit Rate Blocking Rate: {bit_rate_blocking:.4f}")
        print("-" * 40)
